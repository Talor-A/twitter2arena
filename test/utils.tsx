import { RouterContext, BlitzRouter, BlitzProvider, AuthenticatedMiddlewareCtx, Ctx } from "blitz"
...
export function getTestSession({
  user,
  ...rest
}: Partial<AuthenticatedMiddlewareCtx> & { user: User }): AuthenticatedMiddlewareCtx {
  return {
    session: {
      $authorize: jest.fn(),
...
export function getAnonymousTestSession(): Ctx {
  return {
    session: {
      $authorize: jest.fn(),
